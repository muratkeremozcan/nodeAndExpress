...
// NEW 5 replacing repetition with a route instance
var blocksRoute = app.route('/blocks'); // ROUTE INSTANCE using app.route is a recommended approach for avoiding duplicate route names. app.route returns route object which handles all requests to the /blocks path
// no path argument is needed for route handlers belonging to other same route instance
...
//  what if we need to create DYNAMIC ROUTES that need to use the same parameter parsing? 
...
// no path argument is needed for route handlers belonging t othe same route instance
...
// no path argument is needed for route handlers belonging t other same route instance
